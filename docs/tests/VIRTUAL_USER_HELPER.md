...
2. **连续操作**：在同一页面连续操作时，设置 `auto_navigate=False` 避免重复导航

3. **头像上传**：使用 `tests/resources/` 目录下的测试图片

4. **数据清理**：测试结束后删除所有 `TEST_` 前缀的虚拟用户

5. **等待时间**：Helper 内部已包含必要的等待时间，无需额外 sleep

## 注意事项

- 所有 Helper 函数都需要管理员权限
- 切换身份后会留在管理页面，不会自动返回主页
- 创建虚拟用户时会自动处理同名冲突
- 删除操作会自动确认弹窗
