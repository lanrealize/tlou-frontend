...
### 4.1 `test_guest_no_access_state` - 未登录访问私密朋友圈

**测试状态**：guest_no_access（未登录 + 私密朋友圈）
...
**测试步骤**：
- 步骤5：未登录用户通过普通链接进入朋友圈详情页
- 步骤6：验证UI配置：circle-status-action组件不显示（show=false）
- 步骤7：尝试点赞帖子
  - 验证：弹出注册弹窗
  - 验证：提示信息 = "登录后才能点赞"
- 步骤8：关闭弹窗，尝试评论帖子
  - 验证：弹出注册弹窗
  - 验证：提示信息 = "登录后才能发表评论"
- 步骤9：关闭弹窗，尝试进入发布页（enterPublishPage）
  - 验证：弹出注册弹窗
  - 验证：提示信息 = "您需要登录才能发布动态"
- 步骤10：关闭弹窗，尝试进入朋友圈列表页（enterListPage）
  - 验证：弹出注册弹窗
  - 验证：提示信息 = "您需要登录才能查看朋友圈列表"
- 步骤11：尝试创建朋友圈（createCircle）
  - 验证：弹出注册弹窗
  - 验证：提示信息 = "您需要登录才能创建朋友圈"
- 步骤12：尝试进入设置页（enterSettingsPage）
  - 验证：弹出注册弹窗
  - 验证：提示信息 = "您需要登录才能修改设置"
- 步骤13：尝试申请加入（applyToJoin）
  - 验证：弹出注册弹窗
  - 验证：提示信息 = "这是私密朋友圈"
- 步骤14：尝试接受邀请（acceptInvite）
  - 验证：弹出注册弹窗
  - 验证：提示信息 = "这不是邀请链接"
...
### 4.2 `test_guest_can_apply_state` - 未登录访问公开朋友圈

**测试状态**：guest_can_apply（未登录 + 公开朋友圈）
...
- 步骤7：尝试点赞帖子
  - 验证：弹出注册弹窗（rejectAction=showRegisterPopup）
  - 验证：提示信息 = "登录后才能点赞"
  - 验证：不保存意图（saveIntent不包含likePost）
- 步骤8：关闭弹窗，尝试评论帖子
  - 验证：弹出注册弹窗
  - 验证：提示信息 = "登录后才能发表评论"
- 步骤9：关闭弹窗，点击"申请加入"按钮
...
### 4.3 `test_guest_invited_state` - 未登录通过邀请链接访问

**测试状态**：guest_invited（未登录 + 邀请链接）
