...
1. 未注册用户在 main 页面的操作验证
2. 未注册用户进入发现朋友圈并验证状态
3. 未注册用户在 details 页面的操作验证
4. 点击创建朋友圈触发登录
5. 完成用户注册
6. 等待自动创建朋友圈并验证成员状态
7. 作为成员在 details 页面的完整操作
8. 作为注册用户在 main 页面的操作验证
"""
...
def test_unregistered_e2e():
    ...
    测试步骤：
    1. 启动小程序并进入测试模式
    2. 验证未注册用户在 main 页面的操作
    3. 进入发现朋友圈并验证用户状态
    4. 验证未注册用户在 details 页面的操作
    5. 返回 main 页面
    6. 点击创建朋友圈按钮（触发登录）
    7. 完成用户登录
    8. 等待朋友圈创建完成并验证成员状态
    9. 作为成员在 details 页面进行完整操作
    10. 返回 main 页面
    11. 作为注册用户在 main 页面进行操作
    """
    mini = None
    ...
    try:
        ...
        print('\n【步骤3】进入发现朋友圈并验证用户状态')
        print('-'*60)
        enter_result = enter_discover_circle(mini)
        if not enter_result['success']:
            print(f'❌ 步骤3失败：{enter_result["message"]}')
            return False
        
        discover_circle_id = enter_result['circle_id']
        print(f'   ✅ 已进入发现朋友圈: {discover_circle_id[:12]}...')
        
        # 验证未注册用户在发现朋友圈的状态
        status_result = check_circle_status_action(mini, expected_user_status='guest_can_apply')
        if not status_result['match']:
            print(f'❌ 步骤3失败：状态验证失败')
            for error in status_result.get('errors', []):
                print(f'   {error}')
            return False
        print('✅ 步骤3完成')
        
        # 步骤4：验证未注册用户在 details 页面的操作
        print('\n【步骤4】验证未注册用户在 details 页面的操作')
        print('-'*60)
        ...
        if not result['success']:
            print(f'❌ 步骤5失败：{result["message"]}')
            return False
        print('✅ 步骤5完成')
        
        time.sleep(0.5)
        
        # 步骤6：点击创建朋友圈按钮（触发登录弹窗）
        print('\n【步骤6】点击创建朋友圈按钮')
        print('-'*60)
        click_result = click_create_circle_button(mini)
        if not click_result['success']:
            print(f'❌ 步骤6失败：{click_result["message"]}')
            return False
        print('✅ 步骤6完成')
        ...
        if not login_result['success']:
            print(f'❌ 步骤7失败：{login_result["message"]}')
            return False
        
        print(f'✅ 步骤7完成：用户登录成功（{login_result["user_info"]["username"]}）')
        
        # 步骤8：等待朋友圈创建完成并验证成员状态
        print('\n【步骤8】等待朋友圈创建完成并验证成员状态')
        print('-'*60)
        ...
        if not status_result['match']:
            print(f'❌ 步骤8失败：状态验证失败')
            for error in status_result.get('errors', []):
                print(f'   {error}')
            return False
        print('✅ 步骤8完成')
        
        # 步骤9：作为成员在 details 页面进行完整操作
        print('\n【步骤9】作为成员在 details 页面进行完整操作')
        print('-'*60)
